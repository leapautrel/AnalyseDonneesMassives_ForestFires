...
shinyServer(function(input, output) {
  ...
  output$infotimeframe1 <- renderInfoBox({
  	...
  	isolate({
  		...
  		infoBox("Nombre de feux total",
  						nrow(i),
  						icon = icon("fire-smoke"),
  						color = "lime")
  	})
  })
  ...
  output$carte2 <- renderLeaflet({
  	...
  	isolate({
  		...
  		pal2 <-
  			colorBin("YlOrRd", domain = states$fire_count, bins = bins)
...
#   														 order = -1)
# 
#   	# UI 3.3.2 : Infos boxes timeframe ----
#   	output$infotimeframe1 <- renderInfoBox({
#   		infoBox("Nombre de feux total",
#   						nrow(i),
#   						icon = icon("fire-smoke"),
...
#   	})
# 
#   	# Carte 1 : plus gros feux ----
#   	output$carte1 <- renderLeaflet({
#   		# Selection des n plus gros feux
#   		i_carte1 <- i[1:input$nblargestfires, ]
# 
#   		# Creation d'une palette de couleurs
#   		ncauses <- nlevels(i[, stat_cause_descr])
...
# 
#   		# Creation de la carte
#   		m <- leaflet::leaflet(i_carte1) %>%
#   			leaflet::addTiles() %>%
# 
#   			# Centrage de la carte
#   			fitBounds(
#   				lng1 = min(i_carte1[, longitude]),
#   				lat1 = min(i_carte1[, latitude]),
#   				lng2 = max(i_carte1[, longitude]),
#   				lat2 = max(i_carte1[, latitude])
#   			) %>%
# 
#   			# ajout des cercles
#   			leaflet::addCircles(
#   				lng = i_carte1[, longitude],
#   				lat = i_carte1[, latitude],
#   				radius = i_carte1[, fire_size],
#   				weight = 0.2,
#   				stroke = T,
#   				opacity = 100,
#   				fill = T,
